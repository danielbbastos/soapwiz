...
- Consider image downsampling for local image data in performance-sensitive contexts (see `references/image-optimization.md`)
...
- **SwiftData**: Use `@Query` in views only, never in `@Observable` classes or ViewModels
...
| `@Query` | SwiftData queries — views only |
...
- [ ] `@Query` only in views, not in `@Observable` classes or ViewModels
