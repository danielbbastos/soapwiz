...
| `@Query` | SwiftData queries | **Views only** — never in `@Observable` classes |
...
**Critical**: The `@Observable` macro transforms stored properties to add observation tracking. Property wrappers (like `@AppStorage`, `@SceneStorage`) also transform properties with their own storage. These two transformations conflict, causing a compiler error.
...
**`@Query` (SwiftData) must never appear inside an `@Observable` class — not even with `@ObservationIgnored`.** `@Query` is a SwiftUI view macro that connects to the SwiftData model context injected through the environment. It requires a view context to function and has no meaning in a plain class. Always put `@Query` in views.

```swift
// WRONG — @Query in an @Observable class
@Observable
final class IngredientViewModel {
    @ObservationIgnored @Query var ingredients: [Ingredient]  // Won't work
}

// CORRECT — @Query in the view
...
8. **Always add `@ObservationIgnored` to property wrappers** (e.g., `@AppStorage`, `@SceneStorage`) inside `@Observable` classes — they conflict with the macro's property transformation
9. **`@Query` belongs in views only** — never in `@Observable` classes, ViewModels, or services
