...
4. **Fix** slow body work by moving heavy logic into precomputed/cached paths
5. **Use Cause & Effect Graph** to identify unintended update fan-out
6. **Re-record** and compare update counts and hitch frequency

## Example Patterns

- Caching formatted date strings in a model instead of computing in `body`
- Replacing a dependency on a global array with per-item view models to reduce update fan-out
...
| Long Platform View Updates | UIKit hosting in SwiftUI |
