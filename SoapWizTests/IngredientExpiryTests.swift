...
struct IngredientExpiryTests {
    ...
    @Test func returnsNearestWhenMultiplePurchasesExpiring() throws {
        ...
        let purchase1 = makePurchase(expiryDate: sooner)
        let purchase2 = makePurchase(expiryDate: later)
    ...
    @Test func ignoresExpiredWhenMixedWithUpcoming() throws {
        ...
        let purchase1 = makePurchase(expiryDate: past)
        let purchase2 = makePurchase(expiryDate: future)
        ctx.insert(purchase1); ctx.insert(purchase2)
        ingredient.purchases.append(purchase1)
        ingredient.purchases.append(purchase2)
        let result = try #require(ingredient.nearestUpcomingExpiry)
